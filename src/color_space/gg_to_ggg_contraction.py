...
from colorwf_runner import ColorWfResult, parse_color_wf_output, get_singlet_array, get_singlet_coefficients
...
def gg_to_ggg_contraction():
    ...
    # incoming wf
    inc_stdout = run_color_wf_py(0, 0, 2)
    incoming_result = ColorWfResult(n=0, m=0, k=2)
    parse_color_wf_output(inc_stdout, incoming_result)
    ...
    # outgoing coeff
    out_stdout = run_color_wf_py(0, 0, 3)
    outgoing_result = ColorWfResult(n=0, m=0, k=3)
    parse_color_wf_output(out_stdout, outgoing_result)
